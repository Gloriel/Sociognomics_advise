...
file_handler = logging.FileHandler('socio_bot.log')
...
class UserSession:
    ...
    def portrait(self) -> str:
        ...
        return "универсальный интересант"
...
class FinanceBot:
    ...
    def load_questions(self) -> Dict[int, Dict[int, dict]]:
        ...
        csv_path = os.path.join(os.path.dirname(__file__), "questions_socio.csv")
                    ...
                    try:
                        ...
                        if row.get("Выбор пользователя") and row.get("Вариант вопроса"):
                            ...
                            questions[branch][q_id]["options"][choice] = {
                                ...
                                "portrait": row.get("Портрет", "универсальный интересант"),
    ...
    async def show_final_message(self, user_id: int, query: CallbackQuery):
        ...
        if not portrait_description:
            portrait_description = (
                f"<b>Твой социальный профиль: {session.portrait}</b>\n"
                "Ты обладаешь сочетанием разумных интересов, которые позволяют иметь объективную картину мира."
            )
        ...
        final_text = (
            ...
            + "\n\n".join(advice_lines) + "\n\n"
            "<b>Если хочешь повысить свой уровень по жизни, следи за всеми трендами.</b>\n\n"
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        caption = (
            "👋 <b>Добро пожаловать в информационного советника!</b>\n"
            "Этот бот поможет тебе:\n"
            "- Определить твой социальный профиль\n"            
